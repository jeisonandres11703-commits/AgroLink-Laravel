            ...
            <div class="filtros sticky-top" style="top: 10px; background-color: #c6dc93;">
                ...
                <form method="GET" action="{{ route('shipments.dashboard') }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="range" class="form-range peso-slider" min="0" max="15000" step="100" value="0" name="peso">
                    ...
                    <div class="row">
                        <div class="col-md-8">
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6 info-cliente mb-3">
                                    <h6>Cliente</h6>
                                    <p class="mb-1">
                                        <strong>{{ $shipment->purchase->client->user->Name ?? '' }} {{ $shipment->purchase->client->user->last_Name ?? '' }}</strong>
                                    </p>
                                    <p class="mb-1 small">
                                ...
                                <div class="col-md-6 info-vendedor">
                                    <h6>Vendedor</h6>
                                    <p class="mb-1">
                                        <strong>{{ $shipment->purchase->details->first()->product->producer->user->Name ?? '' }} {{ $shipment->purchase->details->first()->product->producer->user->last_Name ?? '' }}</strong>
                                    </p>
                                    <p class="mb-1 small">
                                        <i class="fas fa-phone me-2"></i>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="mb-3">
                                ...
                                <ul class="small ps-3">
                                    <li>Peso: {{ $shipment->purchase->details->first()->product->weight_kg ?? 'N/A' }} kg</li>
                                    <li>Fecha recogida: {{ $shipment->departure_date ?? 'Por definir' }}</li>
                                    <li>Fecha entrega: {{ $shipment->delivery_davte ?? 'Por definir' }}</li>
                                </ul>
                            </div>
                            <div class="d-grid gap-2">
                                <form method="POST" action="{{ route('shipments.accept', $shipment->purchase->id_purchase) }}">
                                    @csrf
                                    <button class="btn btn-aceptar btn-success" type="submit">
                                        <i class="fas fa-check-circle me-2"></i>Aceptar viaje
                                    </button>
                                </form>
                               <a href="{{ route('shipments.show', $shipment->id_shipment) }}" class="btn btn-outline" style="color: #1a512e;">
